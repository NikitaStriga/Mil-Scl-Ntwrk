...
public class PasswordRecoveryController {
    ...
    @ApiOperation(value = "Password recovery request",
            notes = "If login exist then letter will be sent to the user's mail with further instructions")
    @ApiResponses({
            @ApiResponse(code = 200, message = "Mail was sent to user"),
            @ApiResponse(code = 400, message = "Invalid login or email"),
            @ApiResponse(code = 404, message = "User with this login not found")
    })
    @PostMapping
    ...
    @ApiResponses({
            ...
            @ApiResponse(code = 400, message = "Bad recovery code")
    })
    @PostMapping("{code}")
